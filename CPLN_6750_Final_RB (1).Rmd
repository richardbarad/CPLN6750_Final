...
new_nearest_feat <- st_nearest_feature(centroid,new_highways)
```

# Build Fishnet 2021 for scenario 2

```{r}

fishnet_2021_s2 <- cbind(lc2021_extract_df,fishnet_pct_developed %>% select(developed2021)) %>%
  rename(developed = developed2021) %>%
  rowwise() %>% mutate(max = max(c(Forest,Farm,Wetlands,Water,OtherUndeveloped))) %>%
  ungroup() %>%
  mutate(water=as.factor(ifelse(max==Water,1,0)),
...
## Spatial Lag of Development and Road Distance for Scenario 2

``` {r}

fishnet_2021_s2$lagDevelopment_5 <- nn_function(xyC(fishnet_2021_s2),xyC(filter(fishnet_2021_s2,developed==1)),5)
fishnet_2021_s2$lagDevelopment_10 <- nn_function(xyC(fishnet_2021_s2),xyC(filter(fishnet_2021_s2,developed==1)),10)

fishnet_2021_s2$highway_dist <- as.double(st_distance(centroid, new_highways[new_nearest_feat,], by_element=TRUE))

```

## Add Population for Scenario 2

```{r}

fishnet_2021_s2 <-
  st_interpolate_aw(atlantaPop21["pop_2021"], fishnet_2021_s2, extensive=TRUE) %>%
  st_drop_geometry() %>%
  rownames_to_column(var = "ID") %>%
  left_join(fishnet_2021_s2 %>%
              mutate(ID = as.character(ID)),
            ., by=c("ID"='ID')) %>% 
  mutate(pop = replace_na(pop_2021,0))
...
## Join Counties for Scenario 2

``` {r}

fishnet_2021_dev_s2 <- fishnet_2021_s2 %>% filter(developed==1)

fishnet_2021_s2_s2 <- st_join(fishnet_2021_s2,atl_msa %>% rename (county = NAME10) %>% select(county),largest=TRUE) %>%
  filter(water==0) %>%
  filter(developed==0)
...
fishnet_2021_type_s2 <- fishnet_2021_results_s2 %>%
  select(ID,forest,farm,wetlands,otherundeveloped,outcome) %>%
  st_drop_geometry() %>%
  pivot_longer(cols=c(-outcome,-ID),names_to='Type',values_to='Land_Cover') %>%
  filter(Land_Cover == 1) %>%
...
ggplot(data=fishnet_2021_type_s2)+
  geom_bar(aes(x=Type,fill=Type),stat='Count')+
  scale_fill_manual(values=c('yellow','lightgreen','orange','lightblue'),na.value = "transparent",name='Land Cover')+
  ggtitle('Type of Land Cover Grid Squares Predicted to Change')+
  scale_x_discrete('Type of Land Cover')+
